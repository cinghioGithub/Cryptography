...
param = dh.generate_parameters(generator=2, key_size=512, backend=default_backend() )
...
server_secret = private_key.exchange(client_public_key)
client_secret = client_private_key.exchange(public_key)
...
s_key = HKDF(algorithm=SHA256(), length = 16, salt = None, info=server_secret, backend=default_backend()).derive(b'')
for i in range(4):
    print(HKDF(algorithm=SHA256(), length=16, salt=None, info=server_secret, backend=default_backend()).derive(i.to_bytes(1,byteorder='big')))
print(s_key)

c_key = HKDF(algorithm=SHA256(), length = 16, salt = None, info=client_secret, backend=default_backend()).derive(b'')
